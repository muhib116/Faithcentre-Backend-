...
@section('headerTitle', 'All Trashed Slider | '.((isset($site_name)) ? $site_name : ''))
				...
				<div class="table-responsive">					
					...
					<table class="table table-bordered table_filter">
						...
						@foreach($rows as $key => $value)
							<tr>
								<td>{{ $key+1 }}</td>
								<td>{{ is_safe($value->caption) ? $value->caption : '' }}</td>
								<td>{{ is_safe($value->description) ? $value->description : '' }}</td>
								{{-- <td>
									<a href="{{ $value->link }}">{{ $value->anchor_text }}</a> --}}
								<td>
									<a class="btn btn-info" style="white-space: nowrap;" href="{{ $value->link }}">{{ $value->anchor_text }}</a>
								</td>
								<td>
									<img style="height: 60px;" src="{{ asset('public/backend/img/slider/l/'.$value->path) }}" alt="">
								</td>
								<td>
