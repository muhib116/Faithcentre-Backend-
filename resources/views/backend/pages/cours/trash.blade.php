...
@section('headerTitle', 'All Trashed Cours | '.((isset($site_name)) ? $site_name : ''))
					...
					<table class="table table-bordered table_filter">
						...
						@foreach($rows as $key => $value)
							<tr>
								<td>{{ $key+1 }}</td>
								<td>{{ $value->name }}</td>
								<td>{{ make_short(strip_tags($value->detail), 300) }}</td>
								<td><img style="height: 60px;" src="{{ asset('public/backend/images/cours/l/'.$value->image_name) }}" alt=""></td>
