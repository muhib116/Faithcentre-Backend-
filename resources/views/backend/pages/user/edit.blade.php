...
@section('headerTitle', 'Update User | '.((isset($site_name)) ? $site_name : ''))
	...
	<section class="content">
		...
		<div>
			<div class="container_fluid">
				...
				@if($result)
					<form action="{{ route('admin.update_process_user', $result->id).get_system() }}" method="post" enctype="multipart/form-data">
						@csrf
						<div class="col-lg-7 col-12">
							<div class="form-group">
								<label class="label-control">Username</label>
								<input type="text" name="username" Placeholder="Username... *" value="{{ ($result && is_safe($result->username)) ? $result->username : '' }}" required class="form-control">
							</div>
							<div class="form-group">
								<label class="label-control">Email</label>
								<input type="email" name="email" Placeholder="Email... *" value="{{ ($result && is_safe($result->email)) ? $result->email : '' }}" required class="form-control">
							</div>
							<div class="form-group">
								<label class="label-control">Password</label>
								<input type="text" name="password" placeholder="{{ ($result && is_safe($result->password)) ? $result->password : '' }}" class="form-control">
							</div>
							<br>
							...
							{{-- <div>
								...
								<div class="image_preview_wrapper">
									<img style="max-width: 100%;" src="{{ asset('public/backend/img/user/l/'.$result->path) }}" id="preview">
								</div> --}}
							</div>
