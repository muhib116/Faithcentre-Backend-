...
@section('headerTitle', 'Update Students | '.((isset($site_name)) ? $site_name : ''))
		...
		@if($result)
			<div>
				<div class="container_fluid">
					...
					<form action="{{ route('admin.update_process_students', $result->id).get_system() }}" method="post" enctype="multipart/form-data">
						@csrf
						<div class="col-lg-7 col-12">
							...
							<div class="form-group">
								...
								<select name="type" required class="form-control">
									...
									@foreach(student_type() as $type)
										<option {{ ($result->type==$type) ? 'selected' : '' }} value="{{$type}}">{{ ucfirst($type) }}</option>
									@endforeach
								</select>
							</div>
							...
							<div class="form-group">
								<br>
								<br>
								<label class="label-control">Students Image (<small>optional</small>)</label>
								<label class="image_upload_zone" for="">
									...
									<div class="text">
										<span>Drag And drop Students Image</span>
										<strong>Or</strong>
										<br>
										<button>Select Students Image</button>
										<br>
										<span>
											File Type JPG,PNG,Webp
											<br>
											Students Image Size Limit: 300kb
										</span>
									</div>
								</label>
								
								<div class="image_preview_wrapper">
									<img src="{{ asset("public/backend/images/students/l/".$result->image_name) }}" id="image_preview">
								</div>
							</div>
							{{-- Page Students end --}}
