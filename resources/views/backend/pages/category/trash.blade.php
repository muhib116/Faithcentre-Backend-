...
@section('headerTitle', 'All Trashed Category | '.((isset($site_name)) ? $site_name : ''))
				...
				<div class="table-responsive">					
					...
					<table class="table table-bordered table_filter">
						...
						@foreach($rows as $key => $value)
							<tr>
								<td>{{ $key+1 }}</td>
								<td>{{ is_safe($value->name) ? $value->name : '' }}</td>
								{{-- <td><img style="height: 60px;" src="{{ asset('public/backend/images/category/logo/m/'.$value->logo_name) }}" alt=""></td> --}}
								{{-- <td><img style="height: 60px;" src="{{ asset('public/backend/images/category/banner/'.$value->banner_name) }}" alt=""></td> --}}
								<td>
