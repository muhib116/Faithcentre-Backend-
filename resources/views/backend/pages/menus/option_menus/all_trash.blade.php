...
@section('headerTitle', 'All Trashed Option Menu | '.((isset($site_name)) ? $site_name : ''))
			...
			<table class="table table-bordered table_filter">
				...
				@foreach($rows as $key => $row)
					<tr>
						...
						<td>
							<div class="btn-group">
								<a href="{{ route('admin.restore_option_menu', [$row->id]) }}" class="btn btn-primary">
								...
								<form id="delete_form_{{ $row->id }}" action="{{ route('admin.option_menu_parmanently_delete', [$row->id]) }}" method="post" style="display: none;">
									@csrf
								</form>
							</div>
						</td>
