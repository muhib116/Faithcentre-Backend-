...
@section('headerTitle', 'Edit Option Menu | '.((isset($site_name)) ? $site_name : ''))
	...
	<section class="content">
		...
		<div>
			<div class="container_fluid">
				...
				<form action="{{ route('admin.option_menu_edit_process', $result->id) }}" method="post">
					@csrf
					<div>
						<div class="col-lg-4 col-md-6  form-group pl-md-0">
						...
						<div class="col-lg-4 col-md-6  form-group pl-md-0">
							@php
								$dropdown_for_selected_parent = \DB::table("system_aside_menu_dropdowns")
																	->where([
																		"aside_menu_id" => $result->aside_menu_id,
																		"status" => 1
																	])
																	->get();
								// ------------------------------------
							...
							<select name="asidedropdown_menu_id" required class="form-control" id="aside_dropdown">
								...
								@if(count($dropdown_for_selected_parent))
									@foreach($dropdown_for_selected_parent as $key => $value)
										<option {{ ($result->asidedropdown_menu_id === $value->id) ? "selected" : '' }} value="{{ $value->id }}">{{ $value->name }}</option>
									@endforeach
								@endif
							</select>
						</div>
						...
						<div class="col-lg-4 col-md-6  form-group pl-md-0">
							<select name="type" class="form-control man_select" required>
								<option value='' >-Select Option's Type- *</option>
								<option {{ ($result->type == 'success') ? 'selected' : '' }} value="success">Success</option>
								<option {{ ($result->type == 'info') ? 'selected' : '' }} value="info">Info</option>
								<option {{ ($result->type == 'warning') ? 'selected' : '' }} value="warning">Warning</option>
								<option {{ ($result->type == 'danger') ? 'selected' : '' }} value="danger">Danger</option>
								<option {{ ($result->type == 'primary') ? 'selected' : '' }} value="primary">Primary</option>
								<option {{ ($result->type == 'secondary') ? 'selected' : '' }} value="secondary">Secondary</option>
							</select>
						</div>

						<div class="col-lg-4 col-md-6  form-group pl-md-0">
							<input type="text" name="route_name" Placeholder="Option Menu's Route Name... *" value="{{ isset($result->route_name) ? $result->route_name : '' }}" required class="form-control">
						</div>
