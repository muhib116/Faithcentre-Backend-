...
@section('headerTitle', 'All Trashed Aside Menu | '.((isset($site_name)) ? $site_name : ''))
							...
							<tr>
								...
								<td>
									<div class="btn-group m_0">
										<a href="{{ route('admin.restore_aside_menu', [$row->id,$row->has_sub_menu,$row->has_option_menu]) }}" class="btn btn-primary">
											<i class="fa fa-undo"></i>
										</a>
										<button 
											class="btn btn-danger"
											onclick="if(confirm('Are you sure to permanently delete this ?')){document.querySelector('#delete_form_{{ $row->id }}').submit();}else{return false;}" 
										>
											<i class="fa fa-trash"></i>
										</button>
										<form id="delete_form_{{ $row->id }}" action="{{ route('admin.aside_menu_parmanently_delete', [$row->id,$row->has_sub_menu,$row->has_option_menu]) }}" method="post" style="display: none;">
											@csrf
										</form>
									</div>
								</td>
