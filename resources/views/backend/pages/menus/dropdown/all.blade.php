...
@section('headerTitle', 'All Aside Dropdown | '.((isset($site_name)) ? $site_name : ''))
				...
				<table class="table table-bordered table_filter">
					...
					@foreach($rows as $key => $row)
						<tr>
							...
							<td>
								<div class="btn-group">
									<a href="{{ route('admin.aside_menu_dropdown_edit', [$row->id]) }}" class="btn btn-warning">
									...
									<form id="delete_form_{{ $row->id }}" action="{{ route('admin.aside_menu_dropdown_delete', [$row->id]) }}" method="post" style="display: none;">
			                            @csrf
			                        </form>
								</div>
							</td>
