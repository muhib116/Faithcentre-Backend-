...
@section('headerTitle', 'All Trashed Section | '.((isset($site_name)) ? $site_name : ''))
				...
				<div class="table-responsive">					
					...
					<table class="table table-bordered table_filter">
						...
						@foreach($rows as $key => $value)
							<tr>
								<td>{{ $key+1 }}</td>
								<td>{{ ucwords(str_replace("_", " ", $value->section_name)) }}</td>
								<td>{{ $value->title }}</td>
								<td>{{ make_short(strip_tags($value->short_detail), 300) }}</td>
								<td><img style="height: 60px;" src="{{ asset('public/backend/images/page_section/l/'.$value->image_name) }}" alt=""></td>
